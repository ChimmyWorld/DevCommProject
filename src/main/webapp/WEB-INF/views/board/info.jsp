...
<body>
    ...
    <div class="hr"></div>
    <div>
        <h1 class="title1">팁/정보게시판<img class="title1img" src="${cpath }/assetImg/board3.png"></h1>
        </div>

        <div class="write">
            <button class="board-btn board-btn-blue" type="button" onclick="location.href='${cpath }/info/new';" style="cursor: pointer;">
                작성하기
            </button>
            <form action="">
            <div class="dropdown">
                ...
                <div class="dropdown-content">
                    <a href="${cpath }/info?order=idx">최신순</a>
                    <a href="${cpath }/info?order=view_count">조회순</a>
                    <a href="${cpath }/info?order=reply_count">댓글순</a>
                    <a href="${cpath }/info?order=recommend_score">추천순</a>
                </div>
              </div>
            </form>
        </div>
    ...
    <div class="page">  
        <ul>
            <c:if test="${p.prev}">
                <li>&nbsp<a href="${cpath}/info?page=${p.begin - 1}">이전</a></li>
            </c:if>
            <c:forEach var="i" begin="${p.begin}" end="${p.end}">
                <li>&nbsp<a href="${cpath}/info?page=${i}">${i}</a></li>
            </c:forEach>
            <c:if test="${p.next}">
                <li>&nbsp<a href="${cpath}/info?page=${p.end + 1}">다음</a></li>
            </c:if>
        </ul>
    </div>
